...
class Seq2SeqModel(object):
  ...
  def __init__(self,
               ...
               use_lstm=False,
               num_samples=512,  # 用来计算损失的  sample softmax may lead to variable value for same input
               forward_only=False,
               dtype=tf.float32):
    """Create the model.
    ...
    Args:
      ...
      size: number of units in each layer of the model. 隐层的维度
    ...
    def seq2seq_f(encoder_inputs, decoder_inputs, do_decode):
      return embedding_attention_seq2seq(
          ...
          feed_previous=do_decode,  # 表示的是 不进行训练，只是向后传播
          dtype=dtype)
